...
plot(density(log10(connections$neighbour_avg_dist), na.rm = T,
             bw = "SJ"))

# --> bimodal distribution of average distances
...
plot(1:10, f_avg_dist(1:10), type = "b", col = 4)
f_max_dist <- ecdf(connections$distance / connections$neighbour_max_dist)
lines(1:10, f_max_dist(1:10), type = "b", col = 2)
...
q <- "
select id_dcl, gps_id, masts_id, t_start, t_end, ST_X(geom_gps) as gps_x, ST_Y(geom_gps) as gps_y, 
...
library(mgcv)
set.seed(284)
connections <- connections[sample(nrow(connections), nrow(connections) / 4, 
                                  replace = F), ]
connections$radio_level <- factor(connections$radio_level)
model <- gam(distance ~ radio_level + s(neighbour_area) + 
               s(neighbour_max_dist) + s(neighbour_avg_dist),
             data = connections)